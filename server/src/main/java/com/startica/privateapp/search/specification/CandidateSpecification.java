    ...
    public class CandidateSpecification {
        ...
        public static Specification<Candidate> buildSpecification(CandidateSearchRequest request) {
            return (root, query, criteriaBuilder) -> {
                ...
                // Experience filtering is handled in-memory in GlobalSearchService due to string storage
                // Skipping JPA criteria for experience as it's stored as String in database
                ...
                if (request.getMinPercentage() != null) {
                    predicates.add(criteriaBuilder.greaterThanOrEqualTo(root.get("percentage"), request.getMinPercentage()));
                }
                if (request.getMaxPercentage() != null) {
                    predicates.add(criteriaBuilder.lessThanOrEqualTo(root.get("percentage"), request.getMaxPercentage()));
                }
